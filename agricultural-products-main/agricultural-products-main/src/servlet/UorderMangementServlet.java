 ...
 */
@WebServlet("/UorderMangementServlet")
public class UorderMangementServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// TODO Auto-generated method stub
				...
				try {
					String condition="Select * from uorder where  Unum='"+dataBean.getlogname()+"'";
					...
					rowSet.populate(rs);
					dataBean.setRowSetUorder(rowSet);;//行集数据存储到dataBean
					

					con.close();//关闭连接
					}catch(SQLException e) {    
				}
				response.sendRedirect("U-order-management.jsp");
