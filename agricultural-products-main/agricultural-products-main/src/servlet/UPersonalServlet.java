 ...
 */
@WebServlet("/UPersonalServlet")
public class UPersonalServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// TODO Auto-generated method stub
				...
				try {
					condition="select * from user where Unum='"+dataBean.getlogname()+"'";
					...
					rowSet.populate(rs);
					dataBean.setRowSetUUser(rowSet);;//行集数据存储到dataBean
					
					con.close();//关闭连接
					}catch(SQLException e) {    
				}
				response.sendRedirect("U-Personal-center-alter.jsp");
